...
void printerror()
{ // Prints and error message for invalid syntax
...
int isvalidstringcheck(char mystring[100])
{ // Check if string is enclosed within ""
...
struct tm *rdatehandler(char filename[100], int toprint)
{ // Prints last modified date in RFC 5322 format
...
void ddatehandler1(char filename[100], char mystring[100])
{ // Handles date -d "today"/"tomorrow"/"yesterday"
...
void ddatehandler2(char filename[100], char mystring[100])
{ // Handles date -d "next second/minute/hour/week/day/month/year" type commands

    {
        ...
        if (strcmp(second, mystring) == 0)
            lastmodified->tm_sec += 1;
        else if (strcmp(minute, mystring) == 0)
            lastmodified->tm_min += 1;
        else if (strcmp(hour, mystring) == 0)
            lastmodified->tm_hour += 1;
        else if (strcmp(day, mystring) == 0)
            lastmodified->tm_mday += 1;
        else if (strcmp(week, mystring) == 0)
            lastmodified->tm_mday += 7;
        else if (strcmp(month, mystring) == 0)
            lastmodified->tm_mon += 1;
        else if (strcmp(year, mystring) == 0)
            lastmodified->tm_year += 1;
...
void ddatehandler3(char filename[100], char values[100], char times[100])
{ // Handles date -d "+ x second/minute/hour/week/day/month/year" type commands

    {
        ...
        if (strcmp(second, times) == 0)
            lastmodified->tm_sec += value;
        else if (strcmp(minute, times) == 0)
            lastmodified->tm_min += value;
        else if (strcmp(hour, times) == 0)
            lastmodified->tm_hour += value;
        else if (strcmp(day, times) == 0)
            lastmodified->tm_mday += value;
        else if (strcmp(week, times) == 0)
            lastmodified->tm_mday += value;
        else if (strcmp(month, times) == 0)
            lastmodified->tm_mon += value;
        else if (strcmp(year, times) == 0)
            lastmodified->tm_year += value;
        else
            printerror();
...
void ddatehandler4(char filename[100], char values[100], char times[100])
{ // Handles date -d "x second/minute/hour/week/day/month/year ago" type commands
...
void datenoparamhandler(char filename[100])
{ // Handles date [filename] type commands
    if (checkdir(filename) == 1)
    {
        struct stat myfile;
        stat(filename, &myfile);
...
void datecheck(int rows, char inputtokens[rows][100])
{
    ...
    if (isDCommand)
    {
        ...
        else if (rows == 6)
        {
            ...
            if (isnumber(inputtokens[3]))
            {
                ddatehandler4(filename, inputtokens[3], inputtokens[4]);
            }
            else if (isnumber(inputtokens[4]))
            {
