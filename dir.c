...
{
    if (directoryhandlersilent(filename) == 1)
    { // Checks if a file or directory exists with the same name. 0 -- No directory
        if (rmdir(filename) == 0)
        {
...
int main(int argc, char *argv[])
{
    ...
    case 1:; //-r
        ...
        { // Checks if the file exist --> removes the file, does not exist --> Creates a new file
            mkdir(filename, 0777);
